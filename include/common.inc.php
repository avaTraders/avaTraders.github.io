...
//获取顶级栏目英文名字
function getTopCategoryDescription($cid=0) {
global $dsql;
if($cid==0) {
$string ="无效参数";
} else {
$cid=GetTopid($cid);
$sql ="SELECT description FROM dede_arctype where id=".$cid;
$arcRow=$dsql->GetOne($sql);
$string = $arcRow[description];
}
return $string;
}

//获取顶级栏目图片
function getTopCategoryArcpic($cid=0) {
global $dsql;
if($cid==0) {
$string ="无效参数";
} else {
$cid=GetTopid($cid);
$sql ="SELECT arcpic FROM dede_arctype where id=".$cid;
$arcRow=$dsql->GetOne($sql);
$string = $arcRow[arcpic];
}
return $string;
}
